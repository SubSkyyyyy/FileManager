...
def is_video(path):
	if os.path.splitext(path)[-1].lower().strip('.') not in ['mp4', 'mov', 'avi', 'wmv', 'flv', 'mkv', 'mpeg', 'mpg', 'webm', 'rmvb', '3gp', 'ts', 'm4v']:
		return False
	return True


def is_img(path):
	if os.path.splitext(path)[-1].lower().strip('.') not in ['jpg', 'jpeg', 'png', 'git', 'bmp', 'tiff', 'webp', 'raw', 'svg', 'psd', 'heif', 'tif', 'heic']:
		return False
	return True

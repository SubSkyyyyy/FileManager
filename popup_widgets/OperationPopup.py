...
class OperationPopup(Popup):
    ...
    def init_main(self):
        ...
        btn_enter = Button(text=ENTER, size_hint=(0.5, 1.0), font_size=NORMAL_SIZE)
        btn_enter.bind(on_release=self.execute_enter_event)
        btn_close = Button(text=CLOSE, size_hint=(0.5, 1.0), font_size=NORMAL_SIZE)
        ...
        if self.roll_back_able:
            btn_roll_back = Button(text=ROLL_BACK, size_hint=(0.5, 1.0), font_size=NORMAL_SIZE)
            # btn_close.bind(on_release=self.dismiss)
    ...
    def show_finish_pop_up(self, msg='操作完成'):
        ...
        finish_ensure_btn = Button(text='确认', size_hint=(1.0, 0.2), halign='center', font_size=NORMAL_SIZE)

...
from file_operation.file_rename import file_rename, file_suffix_change, rename_roll_back, file_name_to_lower_or_upper
...
class FileRenamePopup(OperationPopup):
    ...
    def prepare_widgets(self):
        ...
        input_box_layout.add_widget(replace_full_name)
        input_box_layout.add_widget(self.replace_full_name_checkbox)

        self.replace_option_dict = {
            '主文件名替换': 'main_file_name_replace',
            '文件名后缀替换': 'file_suffix_replace',
            '主文件名转大写': 'main_file_name_to_upper',
            '主文件名转小写': 'main_file_name_to_lower',
        }
    ...
    def get_params(self):
        ...
        replace_full_name = self.replace_full_name_checkbox.active
        return origin_pattern, target_format, option_selected.text, replace_dir_name, replace_file_name, replace_full_name

    def press_enter(self):
        origin_pattern, target_format, opt, to_replace_dir_name, to_replace_file_name, replace_full_name = self.get_params()
        try:
            if self.replace_option_dict[opt] == 'main_file_name_replace':
                file_rename(self.path_text, origin_pattern, target_format, to_replace_dir_name, to_replace_file_name, replace_full_name)
            elif self.replace_option_dict[opt] == 'file_suffix_replace':
                file_suffix_change(self.path_text, origin_pattern, target_format)
            else:
                file_name_to_lower_or_upper(self.path_text, self.replace_option_dict[opt])
    ...
    def press_roll_back(self, instance):
        try:
            origin_pattern, target_format, opt, replace_dir_name, replace_file_name, replace_full_name = self.get_params()
            rename_roll_back(self.path_text, self.replace_option_dict[opt], origin_pattern, target_format)
            self.show_finish_pop_up('回退完成')
        except Exception as e:
            self.show_error_pop_up(str(e))

    def on_toggle_btn_press(self, instance):
        if self.replace_option_dict[instance.text] != 'file_suffix_replace':
